...
i. What is the remote repository fir this homework?
...
You need to press Staged and then it will automatically become staged. Then you should press Commit and push for it to come fully into github.

iii. How do you commit an issue to the local repository?
First you need to make sure your file is connected to your repository before you start editing the file. You find the link for the file at GitHub Desktop. Then you go to File -> New Prokect -> Version Control -> Git and you paste the link. Then each time you need to go to Git ->  staged -> commit. Then you also need to remember to select push.
...
Perhaps it will help organize different projects and problem sets or keep two different versions of the data.
...
pdd <- filter(polls_us_election_2016, pollster %in% pd)
head(pdd)
...
```
"#$how to make other columns go away??"#$
3.3
```{r}
polls_us_election_2016 %>% arrange(desc(is.na(rawpoll_johnson)))
polls_us_election_2016 %>% arrange(desc(is.na(rawpoll_clinton)))
polls_us_election_2016 %>% arrange(desc(is.na(rawpoll_trump)))
polls_us_election_2016 %>% arrange(desc(is.na(rawpoll_mcmullin)))
